...
task :check_coverage do
  ...
  raise "Spec coverage was not high enough: #{percent.round(2)}%" if percent < min_coverage_threshold

  puts "Nice job!  Spec coverage is still at least #{min_coverage_threshold}%"
end

task default: %i[spec check_coverage cane]
...
namespace :core do
  ...
  task update: %i[update_submodule build]
  ...
  namespace :verify do
    ...
    task :current do
      ...
      raise 'The current scripts are out of date with qless-core' unless our_md5s == canonical_md5s
    end
  end
